...
var TOOLS_MODE_ALL_HIDDEN	=	'ALL_HIDDEN';
 ...
 {
	if (thumbItem.mrImage.mType != 'album')
	{
		thumbItem.SelectSlide();
	}
	else
	{
		window.location = thumbItem.mrImage.mSrc;
	}
...
function JphSlider_Slide( preloader, queue, image)
 ...
 JphSlider_Slide.prototype._GetImage = function()
 {
 	...
 	{
		if (this.mrImage.mType == 'video') 
		{
			img		=	this.mrImageQueue.GetVideo(this.mrImage.mSrcThumb);
		}
		else
		{
...
function JphSlider_ImageQueue( queueSize)
 ...
 JphSlider_ImageQueue.prototype.GetVideo        =       function(strThumb)
 {
        ...
        return this._CreateVideo(strThumb);
 }
 ...
 JphSlider_ImageQueue.prototype._CreateVideo    =       function(strThumb)
 {
        var img =       document.createElement('video');
	img.setAttribute("controls","controls");
	img.setAttribute("poster", strThumb);
        this.maLruQueue[this.maLruQueue.length] =       img;
        return img;
 }
...
function Jph_Image( index, src, thumbSrc, title, desc, type, thumbstyle)
 {
 	if (title==undefined)
		title =	'';
 	if (desc==undefined)
 ...
 Jph_Dao.prototype.ReadImages	=	function()
{
	...
	do
	{
		...
		var thumbstyle		=	arguments[i++];
		
		this.AddImage( new Jph_Image( id, src, src_thumb, title, desc, type, thumbstyle));	
	}
	while (i<arguments.length) 
}

 Jph_Dao.prototype.ReadImage		=	function( id, src, thumbSrc, title, desc, type, thumbstyle)
{
// function Jph_Image( index, src, thumbSrc, title, desc, type, thumbstyle)	
